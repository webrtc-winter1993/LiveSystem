				...
				<el-form :inline="true" :model="state.formInline" class="demo-form-inline">
				  <el-form-item label="选择摄像头">
				    <el-select v-model="state.formInline.videoId" placeholder="摄像头">
				      <el-option v-for="(item,index) in useDevice.localDevice.videoIn " :key="index" :label="item.label" :value="item.id"></el-option>
				    </el-select>
				  </el-form-item>
				  <el-form-item label="选择麦克风">
				    <el-select v-model="state.formInline.audioInId" placeholder="麦克风">
				      <el-option v-for="(item,index) in useDevice.localDevice.audioIn " :key="index" :label="item.label" :value="item.id"></el-option>
				    </el-select>
				  </el-form-item>
				  <el-form-item label="选择听筒">
				    <el-select v-model="state.formInline.audioOutId" placeholder="听筒">
				      <el-option v-for="(item,index) in useDevice.localDevice.audioOut " :key="index" :label="item.label" :value="item.id"></el-option>
				    </el-select>
				  </el-form-item>
...
const onSubmit = async () => {
  ...
  let newStream = await getTargetDeviceMedia(state.formInline.videoId, state.formInline.audioInId,state.formInline);
